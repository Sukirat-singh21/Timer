...
function dkey(date) { return date.toISOString().slice(0, 10); }
...
function addRecord({subject, questions, note, minutes}) {
...
function buildWeeklyBuckets() {
  ...
  const first = parseDateKey(recs[recs.length - 1].date); // oldest record (records sorted newest first)
  const today = startOfToday();
...
function buildMonthBuckets() {
  ...
  for (let i = 0; i < weekCount; i++) {
    ...
    buckets.push({ label: `Week ${i + 1}`, start, end, records });
  }
  return buckets;
}
function makeDayRows(bucket) {
  ...
  for (let i = 0; i < 7; i++) {
    ...
    const bySubject = SUBJECTS.reduce((acc, s) => {
      acc[s] = dayRecords.filter(r => r.subject === s).reduce((a, r) => a + (Number(r.minutes) || 0), 0);
      return acc;
    }, {});
    days.push({
      key,
      ...
      totalMinutes: dayRecords.reduce((a, r) => a + (Number(r.minutes) || 0), 0),
      questions: dayRecords.reduce((a, r) => a + (Number(r.questions) || 0), 0),
      bySubject
    });
  }
  return days;
...
function renderAnalytics() {
  ...
  els.periodLabel.textContent = selected ? selected.label : (view === 'weekly' ? 'Week 1' : 'Month week 1');
  renderPeriodButtons();
  ...
  if (state.analyticsView === 'weekly') {
    const days = makeDayRows(selected);
    const maxTotal = Math.max(1, ...days.map(d => d.totalMinutes));
    els.graphArea.innerHTML = days.map(day => {
      const total = day.totalMinutes;
      const height = Math.max(10, Math.round((total / maxTotal) * 190));
      const segments = SUBJECTS.filter(s => day.bySubject[s] > 0).map(sub => {
        const segH = Math.max(6, Math.round((day.bySubject[sub] / Math.max(total, 1)) * height));
        return `<div class="segment ${subjectColorClass(sub)}" style="height:${segH}px" title="${sub}: ${day.bySubject[sub]}m"></div>`;
      }).join('');
      return `
        <div class="day-col">
          <div class="bar-wrap">
            <div class="bar" style="height:${height}px" title="${day.label} • ${total}m • ${day.questions}q">
              <div class="bar-total">${total ? `${total}m` : '0m'}</div>
              ${segments || '<div class="segment" style="height:100%;background:rgba(148,163,184,.16)"></div>'}
            </div>
          </div>
          <div class="day-name">${day.short}</div>
          <div class="day-qs">${day.questions} q</div>
        </div>
      `;
    }).join('');
  } else {
    const weeks = buckets;
    const maxTotal = Math.max(1, ...weeks.map(w => w.records.reduce((a, r) => a + (Number(r.minutes) || 0), 0)));
    els.graphArea.innerHTML = weeks.map(week => {
      const totals = SUBJECTS.reduce((acc, s) => {
        acc[s] = week.records.filter(r => r.subject === s).reduce((a, r) => a + (Number(r.minutes) || 0), 0);
        return acc;
      }, {});
      const total = Object.values(totals).reduce((a, b) => a + b, 0);
      const q = week.records.reduce((a, r) => a + (Number(r.questions) || 0), 0);
      const height = Math.max(10, Math.round((total / maxTotal) * 190));
      const segments = SUBJECTS.filter(s => totals[s] > 0).map(sub => {
        const segH = Math.max(6, Math.round((totals[sub] / Math.max(total, 1)) * height));
        return `<div class="segment ${subjectColorClass(sub)}" style="height:${segH}px" title="${sub}: ${totals[sub]}m"></div>`;
      }).join('');
      return `
        <div class="day-col">
          <div class="bar-wrap">
            <div class="bar" style="height:${height}px" title="${week.label} • ${total}m • ${q}q">
              <div class="bar-total">${total ? `${total}m` : '0m'}</div>
              ${segments || '<div class="segment" style="height:100%;background:rgba(148,163,184,.16)"></div>'}
            </div>
          </div>
          <div class="day-name">${week.label}</div>
          <div class="day-qs">${q} q</div>
        </div>
      `;
    }).join('');
  }
